        ...
        if "dashboard" in driver.current_url:
            ...
            for _ in range(3):
                try:
                    ...
                    try:
                        claim_btns = driver.find_elements(By.XPATH, "//a[contains(@href,'/account/reward/earn')]")
                        if any(el.is_displayed() for el in claim_btns):
                            logger.info("检测到‘领取奖励’，进入签到流程")
                        else:
                            completed = driver.find_elements(By.XPATH, "//span[contains(text(),'已完成') and ancestor::*[contains(.,'每日签到')]]")
